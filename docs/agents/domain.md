...
`docs/adr/` at the repo root. No monorepo signals in this project (single
...
- Read `CONTEXT.md` before making a change that touches domain
  ...
  workflow) — it's the canonical vocabulary, not `docs/architecture.md`
...
- Check `docs/adr/` for a relevant decision record before re-deciding
  something structural (storage split, event pattern, auth boundary) that
  may already have a recorded rationale.
- `CONTEXT.md` and `docs/adr/0001-network-model-not-hub-and-spoke.md` were
